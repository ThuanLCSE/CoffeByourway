export const hostServer = 'http://169.254.148.213:3013/'
export const getAllMainType = hostServer + 'api/coffee/mainType'
export const getSecondRecipeByType = hostServer + 'api/coffee/secondRecipe/'//:drinkId'
export const getThirdRecipeByType = hostServer + 'api/coffee/thirdRecipe/'//:drinkId'
export const managerSignIn = hostServer + 'api/mng/signIn'
export const managerSignUp = hostServer + 'api/mng/signUp'
export const managerSignOut = hostServer + 'api/mng/signOut'
export const createIngredient = hostServer + 'api/ingre/create'
export const removeIngredient = hostServer + 'api/ingre/remove'

export const listAllIngredient = hostServer + 'api/ingre/getAll'
export const uploadFileUrl = hostServer + 'api/upload' 
export const createDrink = hostServer + 'api/drink/create'
export const checkSignIn = hostServer + 'api/mng/checkSignIn'
export const createCustomCup = hostServer +'api/custom/create'
export const createPattern = hostServer + 'api/patternCtrl/create'

export const listAllPattern = hostServer + 'api/patternCtrl/getAll' 


